...
ot = OrbitalTransformation(
    ...
    Ω = 0°,
    plx = 6.8,
    platescale = 785.7/(642/2), # mas/px from scalebar
    dt = 50*365.0
)
...
##
heatmap(collect.(axes(out))...,collect(out), axis=(;aspect=DataAspect()),  colormap=:magma,colorrange=(-0.1, 1.2))
...
img_w = @lift begin
    ...
    out = centered(warp(img, ot, axes(img),fill=0))
...
# annotations!(ax, "HL Tau\nALMA", color=:white, position=(10,3), textsize=32)
# annotations!(
#     ax,
#     @lift(
...
##
for td in Date("2010"):Month(12):Date("2040")
    target_date[] = td
    sleep(0.1)
end
##
