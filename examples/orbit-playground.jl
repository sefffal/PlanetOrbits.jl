...
using Revise, PlutoUI, PlanetOrbits, Plots; theme(:ggplot2)
...
md"""
*Specify the Keplerian elements to visualize using the draggable fields below.*
"""

# ╔═╡ c179bd9e-e392-4622-92a5-d64f442e2840
md"""
`plx` = $(@bind plx Scrubbable(10:1:100, default=25))mas,
`M` = $(@bind M Scrubbable(0.1:0.01:5, default=1))M⊙,
`a` = $(@bind a Scrubbable(0.1:0.1:50, default=5))au, 
`i` = $(@bind i Scrubbable(0:1:180.0, default=1))°, 
`e` = $( @bind e Scrubbable(0:0.01:3, default=0)), 
"""

# ╔═╡ 387f4465-98e1-429f-96f6-490a850f877b
md"""
show planet $(@bind showplanet CheckBox())
"""

# ╔═╡ 82635a9a-8d78-46f9-b9ac-91f5f78837e2
md"""
`Ω` = $(@bind Ω Scrubbable(0:2:360.0, default=0))°, 
`ω` = $(@bind ω Scrubbable(0:2:360.0, default=0))°,
`tp` = $(@bind tp Scrubbable(0:50:10000.0, default=0)) days
"""

# ╔═╡ dde3ae5c-c51d-4efd-a824-3e360981a228
md"""
Time= $(@bind t Scrubbable(0:4:1000, default=0)) mjd
"""

# ╔═╡ 3f38e8ca-2286-427f-8816-3b8b6cc78c74
elem = orbit(;a,i=deg2rad(i),e,tp,M,ω=deg2rad(ω),Ω=deg2rad(Ω),plx)
...
if isfinite(period(elem))
	ts = range(-period(elem), period(elem), step=2period(elem)÷150)
else
	ts = range(-600, 600, length=450)
end

# ╔═╡ e8d619bc-e37d-437a-96fb-0995aed2f823
begin
	sols = orbitsolve.(elem, ts)
	ra = raoff.(sols)
	dec = decoff.(sols)
	zoff = posz.(sols)
	rv = radvel.(sols)
...
begin 
	...
	p1 = plot(xflip=true, aspectratio=1, legendl=:bottomright,  size=(650,400), fontfamily="", fmt=:svg)
	scatter!([0], [0], label="star", marker=:star, color=3, ms=10)
	l = maximum(sqrt.(ra.^2 .+ dec.^2))*1.05
	plot(elem,label="orbit" )
	xlims!(-1.4l,1.4l)
	ylims!(-l,l)
	if showplanet
		scatter!([raoff(elem, t)], [decoff(elem, t)], label="planet", ms=10, color=2)
	end
	scatter!([0], [0], label="star", ms=10, color=:gold, marker=:star)
	xlabel!("ΔRA - mas")
	ylabel!("ΔDEC - mas")
end
...
# ╔═╡ e3607cc4-bfac-4d1d-bb34-e8ef0d42a748
...
manifest_format = "2.0"
project_hash = "26edd5c3c5e8130f71f105c88d88a26fb141a19f"
...
[[deps.PlanetOrbits]]
deps = ["AstroTime", "ChainRulesCore", "CoordinateTransformations", "Dates", "LinearAlgebra", "NamedTupleTools", "PrecompileTools", "Random", "RecipesBase", "Roots", "StaticArrays"]
git-tree-sha1 = "de2d8b465353ccc4d8999ae59f1805b14fc3b8b6"
uuid = "fd6f9641-d78f-43ce-a379-ceb0bddb468a"
version = "0.7.0"

    [deps.PlanetOrbits.extensions]
    PlanetOrbitsEnzymeExt = "Enzyme"
    PlanetOrbitsForwardDiffExt = "ForwardDiff"
    PlanetOrbitsMakieExt = "Makie"
    PlanetOrbitsSymbolicsExt = "Symbolics"

    [deps.PlanetOrbits.weakdeps]
